...
export default function App() {
  ...
  const handlePlanChange = (planId: ZapierPlanId) => {
    ...
    if (plan) { setMonthlyTasks(plan.defaultTasks); setCustomPrice(''); }
  };

  const parsedCustomPrice = customPrice !== '' ? parseFloat(customPrice) : undefined;
  const results = calculateSavings(selectedPlanId, monthlyTasks, isNaN(parsedCustomPrice!) ? undefined : parsedCustomPrice, selectedMakeTier);
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-900 font-sans flex flex-col antialiased selection:bg-violet-600 selection:text-white">
      <Header onOpenMigrationForm={() => setIsMigrationModalOpen(true)} />
      <main className="flex-1 max-w-6xl w-full mx-auto px-4 sm:px-8 py-8 sm:py-10 space-y-8">
        <div className="text-center max-w-3xl mx-auto space-y-3">
          ...
          <div className="flex flex-wrap items-center justify-center gap-4 text-[11px] font-semibold text-slate-500 uppercase tracking-wider pt-1">
            <span className="flex items-center gap-1.5"><CheckCircle2 className="w-3.5 h-3.5 text-violet-600" />100% Free Instant ROI</span>
            <span className="flex items-center gap-1.5"><ShieldCheck className="w-3.5 h-3.5 text-violet-600" />July 2026 Pricing Benchmarks</span>
            <span className="flex items-center gap-1.5"><TrendingDown className="w-3.5 h-3.5 text-violet-600" />1:1.2 Safety Conversion Ratio</span>
          </div>
        </div>
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
          <div className="lg:col-span-5">
            <CalculatorInputs selectedPlanId={selectedPlanId} onPlanChange={handlePlanChange} monthlyTasks={monthlyTasks} onTasksChange={setMonthlyTasks} customPrice={customPrice} onCustomPriceChange={setCustomPrice} />
          </div>
          <div className="lg:col-span-7">
            <SavingsResults results={results} selectedMakeTier={selectedMakeTier} onMakeTierChange={setSelectedMakeTier} />
          </div>
        </div>
        <CTAButtons onOpenMigrationForm={() => setIsMigrationModalOpen(true)} monthlySavings={results.monthlySavings} />
        <ComparisonDetails />
        <SeoContentBlock />
        <FAQSection />
      </main>
      <footer className="border-t border-slate-200 bg-white py-8 mt-12 text-xs text-slate-500">
        <div className="max-w-6xl mx-auto px-4 sm:px-8 space-y-4">
          <div className="flex flex-col sm:flex-row items-center justify-between gap-4 text-center sm:text-left font-medium">
            <p className="tracking-wide text-[11px]">Make Automation Calculator</p>
            <div className="flex items-center gap-4 text-slate-600 text-[11px] uppercase font-bold tracking-wider">
              <button onClick={() => setIsMigrationModalOpen(true)} className="hover:text-violet-600 transition">Free Migration</button>
              <a href="https://www.fiverr.com/s/42e3D4m" target="_blank" rel="noopener noreferrer" className="hover:text-violet-600 transition">Fiverr Service</a>
              <a href="https://www.make.com/en/register?pc=automakecalc" target="_blank" rel="noopener noreferrer" className="hover:text-violet-600 transition">Make.com</a>
            </div>
          </div>
          <div className="pt-3 border-t border-slate-100 text-[10px] text-slate-400 text-center sm:text-left leading-relaxed">
            This page contains an affiliate link to Make.com. Signing up through it gets you a free 1-month trial of the Pro plan (10,000 credits, no cost), after which your account moves to the Free plan unless you choose to continue on a paid tier. I may earn a commission at no extra cost to you. Pricing figures verified as of July 2026.
          </div>
        </div>
      </footer>
      <FreeMigrationModal isOpen={isMigrationModalOpen} onClose={() => setIsMigrationModalOpen(false)} selectedPlanLabel={selectedPlanLabel} monthlyTasks={monthlyTasks} monthlySavings={results.monthlySavings} onLeadSubmitted={() => {}} />
    </div>
  );
}